      ...
      <template v-for="(card, index) in cardData" :key="index">
        ...
        >
          <div class="relative flex justify-between gap-3 text-white card__header">
            <span class="text-[--foreground] text-xl font-bold card__label">{{ card.label }}</span>
          </div>
          <div class="relative flex flex-col text-white card__content">
            <h3 :class="`card__title font-bold text-lg m-0 mb-1 text-[--foreground] ${textAutoHide ? 'text-clamp-1' : ''}`">
              {{ card.title }}
            </h3>
            <p :class="`card__description text-sm leading-5 opacity-90 text-[--muted-foreground] ${textAutoHide ? 'text-clamp-2' : ''}`">
              {{ card.description }}
            </p>
          </div>
        </ParticleCard>
        ...
        >
          <div class="relative flex justify-between gap-3 text-[--foreground] card__header">
            <span class="text-xl font-bold card__label">{{ card.label }}</span>
          </div>
          <div class="relative flex flex-col text-[--foreground] card__content">
            <h3 :class="`card__title font-bold text-lg m-0 mb-1 ${textAutoHide ? 'text-clamp-1' : ''}`">
              {{ card.title }}
            </h3>
            <p :class="`card__description text-sm leading-5 opacity-90 text-[--muted-foreground] ${textAutoHide ? 'text-clamp-2' : ''}`">
              {{ card.description }}
