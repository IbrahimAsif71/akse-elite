...
const ParticleCard = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const initializeParticles = () => {
      ...
      memoizedParticles.value = Array.from(
  ...
  setup(props) {
    ...
    const setupEventListeners = () => {
      ...
      const handleMouseMove = (e: MouseEvent) => {
        ...
        const cards = props.gridRef.querySelectorAll(
          ".card",
      ...
      const handleMouseLeave = () => {
        ...
        (
          props.gridRef?.querySelectorAll(".card") as NodeListOf<HTMLDivElement>
        ).forEach((card) => {
          card.style.setProperty("--glow-intensity", "0");
        });
  ...
  >
    <div class="gap-2 grid card-responsive">
      <template v-for="(card, index) in cardData" :key="index">
        ...
        >
          <div
            class="relative flex justify-between gap-3 text-white card__header"
          >
            <span class="text-[--foreground] text-xl font-bold card__label">{{
              card.label
            }}</span>
          </div>
          <div class="relative flex flex-col text-white card__content">
            <h3
              :class="`card__title font-bold text-lg m-0 mb-1 text-[--foreground] ${textAutoHide ? 'text-clamp-1' : ''}`"
            >
              {{ card.title }}
            </h3>
            <p
              :class="`card__description text-sm leading-5 opacity-90 text-[--muted-foreground] ${textAutoHide ? 'text-clamp-2' : ''}`"
            >
              {{ card.description }}
            </p>
          </div>
        ...
        >
          <div
            class="relative flex justify-between gap-3 text-[--foreground] card__header"
          >
            <span class="text-xl font-bold card__label">{{ card.label }}</span>
          </div>
          <div class="relative flex flex-col text-[--foreground] card__content">
            <h3
              :class="`card__title font-bold text-lg m-0 mb-1 ${textAutoHide ? 'text-clamp-1' : ''}`"
            >
              {{ card.title }}
            </h3>
            <p
